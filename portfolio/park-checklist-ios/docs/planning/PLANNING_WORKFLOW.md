# Park Checklist — planning workflow
...
When a planning item becomes code: **Linear issue** (or update) with acceptance criteria + link to the filled doc section; on ship, **CHANGELOG** + **ROADMAP** in `portfolio/park-checklist-ios/`, and a row in repo root **ROADMAP.md** when the change is portfolio-visible.

**Web apps (Wellness, Job Search, App Forge, RollerTask):** push/PR may run **`.github/workflows/portfolio-web-build.yml`** — keep **`npm run build`** green when you touch those folders.
