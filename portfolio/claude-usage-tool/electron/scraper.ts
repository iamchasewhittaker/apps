...
function stripElectronUA(webContents: Electron.WebContents) {
  const ua = webContents.getUserAgent().replace(/Electron\/\S+\s*/g, '');
...
// platformLoginWindow removed — platform.claude.com crashes in Electron, so we open in system browser
...
export async function scrapeClaudeUsage(): Promise<ClaudeMaxUsage | null> {
  ...
  return new Promise((resolve) => {
    ...
    stripElectronUA(scraperWindow.webContents);
    ...
    scraperWindow.webContents.on('did-finish-load', async () => {
      ...
      try {
        const jsTimeout = new Promise<never>((_, reject) =>
          setTimeout(() => reject(new Error('JS execution timeout')), 5000)
        );
        const result = await Promise.race([scraperWindow.webContents.executeJavaScript(`
          (function() {
        ...
        if (result) {
          ...
          if (parsed.bars?.length === 0 && parsed.isAuthenticated) {
            console.log('No bars found. Full page text:\n', parsed.rawText);
          }
...
export async function scrapeBillingInfo(): Promise<BillingInfo | null> {
  ...
  return new Promise((resolve) => {
    ...
    stripElectronUA(billingWindow.webContents);
    ...
    billingWindow.webContents.on('did-finish-load', async () => {
      ...
      // Track any navigations that occur during the wait
      let navigatedDuringWait = false;
      const navListener = (_: Electron.Event, url: string) => {
        console.log('Billing navigated during wait to:', url);
        navigatedDuringWait = true;
      };
      billingWindow.webContents.on('did-navigate', navListener);
      billingWindow.webContents.on('did-navigate-in-page', navListener);

      // Wait for JavaScript to render
      await new Promise(r => setTimeout(r, 7000));

      billingWindow.webContents.removeListener('did-navigate', navListener);
      billingWindow.webContents.removeListener('did-navigate-in-page', navListener);

      const currentBillingUrl = billingWindow?.webContents.getURL() || '';
      console.log('Billing after wait - navigated:', navigatedDuringWait, 'url:', currentBillingUrl);

      if (resolved || !billingWindow || billingWindow.isDestroyed()) return;

      // If the page redirected to login during the wait, bail out
      if (currentBillingUrl.includes('/login') || currentBillingUrl.includes('/signup')) {
        console.log('Billing redirected to login - not authenticated');
        resolved = true;
        isScrapingBilling = false;
        clearTimeout(timeout);
        if (billingWindow && !billingWindow.isDestroyed()) {
      ...
      try {
        ...
        const result = await Promise.race([billingWindow.webContents.executeJavaScript(`
          (function() {
            const billing = {
              creditBalance: null,
              currency: 'USD',
...
export async function openPlatformLoginWindow(): Promise<boolean> {
  // platform.claude.com crashes in Electron's embedded Chromium,
  // so open it in the user's default browser instead
  await shell.openExternal('https://console.anthropic.com/settings/billing');
  console.log('Opened platform billing in system browser');
  return true;
}
