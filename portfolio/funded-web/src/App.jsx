...
function LoginScreen() {
  ...
  const signInWithGoogle = async () => {
    setError("");
    const { error: e } = await auth.signInWithOAuth({ provider: "google", options: { redirectTo: window.location.origin } });
  ...
  const s = {
    ...
    btnGoogle: { width: "100%", padding: "12px", borderRadius: 8, background: T.surfaceHigh, border: `1px solid ${T.border}`, color: T.text, fontWeight: 600, fontSize: 15, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 },
    divider: { display: "flex", alignItems: "center", gap: 10, margin: "16px 0", color: T.muted, fontSize: 12 },
      ...
      <div style={s.card}>
        ...
        {!sent ? (
          <>
            ...
            <div style={s.divider}><span style={s.dividerLine} /><span>or</span><span style={s.dividerLine} /></div>
