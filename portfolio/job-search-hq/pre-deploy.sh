...
SAVE_COUNT=$(grep "localStorage.setItem(" "$APP_FILE" 2>/dev/null | grep -v "pre-deploy-ignore" | wc -l | tr -d " " || echo 0)
...
if grep -q "chase_" "$APP_FILE"; then
  KEY=$(grep -o '"chase_[^"]*"' "$APP_FILE" | head -1)
