...
| **Last touch** | 2026-04-28 — Gmail Forge × JSHQ fully verified: trigger active, `JobSearch` label exists, 3-pass match logic deployed, LinkedIn social split (connection suggestions → Notification, not JobSearch). SetupGuide callout updated to reference `healthCheck_jobSearch` (no trailing underscore). System ready — next real job email will populate InboxPanel. — Prior: SetupGuide "Using Gmail Forge?" callout + `subjectPatterns` + Gemini prompt fix. — Prior: 2026-04-26 v8.18 ship: `api/gmail/{exchange,refresh,disconnect}.js` + `api/_lib/{crypto,supabase}.js`; `src/inbox/{oauth,gmailClient,classifier,syncInbox}.js`; `src/components/{InboxPanel,InboxItem}.jsx`; `inbox: []` + `INBOX_KINDS` + helpers + 28 style tokens; App.jsx inbox handlers + `onAfterSave` modal hook. Build clean (+8.73 kB). |
