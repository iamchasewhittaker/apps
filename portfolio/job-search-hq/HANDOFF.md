...
| **Last touch** | 2026-04-28 — Setup-guide polish: `InboxPanel.jsx` `SetupGuide()` now leads with a "Using Gmail Forge?" callout (label is auto-applied; verify via `healthCheck_jobSearch_()`). Build clean (+162 B). Companion Gmail Forge changes in same session add `subjectPatterns` to the JobSearch rule, fix the Gemini prompt to include `JobSearch`, and add `healthCheck_jobSearch_()` diagnostic. — Prior: 2026-04-26 v8.18 ship: `api/gmail/{exchange,refresh,disconnect}.js` + `api/_lib/{crypto,supabase}.js`; `src/inbox/{oauth,gmailClient,classifier,syncInbox}.js`; `src/components/{InboxPanel,InboxItem}.jsx`; `inbox: []` + `INBOX_KINDS` + helpers + 28 style tokens; App.jsx inbox handlers + `onAfterSave` modal hook. Build clean (+8.73 kB). |
