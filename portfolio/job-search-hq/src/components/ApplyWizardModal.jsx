      ...
      <div style={s.wizModal}>
        <div style={s.modalHeader}>
          <div>
            ...
            <div style={{ fontSize: 12, color: T.muted, fontWeight: 400, marginTop: 2 }}>
          ...
          {step === 1 && (
            <>
              <div style={s.wizPrompt}>
                Walking <strong style={{ color: T.foreground }}>{app.company || "this app"}</strong> from
                Interested to Applied. Should take 8–12 minutes if you have the JD ready.
              </div>
              <div style={{ background: T.cardSubtle, border: `1px solid ${T.border}`, borderRadius: 8, padding: 12, fontSize: 13, color: T.foreground, lineHeight: 1.6 }}>
                <div><strong style={{ color: T.foreground }}>Company:</strong> {app.company || "—"}</div>
                <div><strong style={{ color: T.foreground }}>Title:</strong> {app.title || "—"}</div>
                {app.url && (
                  <div style={{ marginTop: 4 }}>
                    <strong style={{ color: T.foreground }}>URL:</strong>{" "}
          ...
          {step === 3 && (
            <>
              ...
              <div style={{ fontSize: 11, color: T.muted, lineHeight: 1.5 }}>
                Resume type: <strong style={{ color: T.muted }}>{resumeType}</strong> (primary).
          ...
          {step === 5 && (
            <>
              ...
              {app.url ? (
                ...
                <div style={{ background: T.goldDarkBg, border: `1px solid ${T.goldDarkBorder}`, borderRadius: 8, padding: 12, fontSize: 12, color: T.warning }}>
                  No URL on this app. Apply on the company's career site, then come back.
                </div>
              )}
              <button onClick={markApplied} style={{ ...s.wizCta, background: T.offerGreenBg }}>
          ...
          {step === 7 && (
            <>
              ...
              {nextQueue.length > 0 ? (
                <>
                  <div style={s.wizPrompt}>
                    Next in queue: <strong style={{ color: T.foreground }}>{nextQueue[0].company}</strong>
                    {" — "}{nextQueue[0].title || "(no title)"}.
                    {nextQueue[0].jobDescription
                      ? <span style={{ color: T.meeting }}> JD ready ✓</span>
                      : <span style={{ color: T.muted }}> No JD yet</span>}
                  </div>
