...
Spend Clarity lives in the sibling folder [`../spend-clarity/`](../spend-clarity/) (same parent as this `inbox-zero` repo). It enriches YNAB transactions using Gmail receipts.

## Contract

1. **Inbox Zero** labels transactional mail with the user-visible label **Receipt** (XML filters + Apps Script).
...
Keep Receipt coverage aligned: when Spend Clarity adds a merchant sender, add the same domain or address to Inbox Zero (`gmail-filters.xml` and `apps-script/rules.gs`).

## Run Spend Clarity after Inbox Zero has labeled mail
...
(From `inbox-zero/integrations/`, use `cd ../../spend-clarity` instead.)
