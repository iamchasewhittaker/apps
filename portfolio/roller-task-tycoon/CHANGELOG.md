...
- **Archived:** moved to `portfolio/archive/roller-task-tycoon`; maintained RollerTask Tycoon surface is **iOS** at `portfolio/roller-task-tycoon-ios`. Removed from portfolio web CI.
- **Monorepo (historical):** when this lived at `portfolio/roller-task-tycoon`, Vercel **Root Directory** matched that path. Linear project description updated.
