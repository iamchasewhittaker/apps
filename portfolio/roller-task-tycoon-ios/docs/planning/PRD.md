...
- User can export backup JSON via system share sheet; filename pattern `RollerTaskTycoon-backup-YYYY-MM-DD.json`.
...
**Schema:** Import **only** when `schemaVersion == 1` (matches [`BackupFormat.schemaVersion`](../../RollerTaskTycoon/Data/BackupSupport.swift)). **Higher** schema → error (“newer app required”). **Lower or other** unsupported values → error (no silent migration in v1).
