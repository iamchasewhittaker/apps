# Technical Design — RollerTask Tycoon (iOS)
...
| Preferences | `@AppStorage` → `UserDefaults` (`chase_roller_task_tycoon_ios_*`; migrated from `chase_park_checklist_ios_*` once) |
| Networking | None (v1) |
| Backend | None (v1) |
| Analytics | None (v1) |
...
- `RollerTaskTycoonApp.swift` — app entry, model container, prefs migration.
...
- Code: [`BackupSupport.swift`](../../RollerTaskTycoon/Data/BackupSupport.swift)
