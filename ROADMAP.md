...
| 2026-04-12 | YNAB Clarity (iOS) | v0.2 | **UX + accessibility:** new app icon; `PayeeDisplayFormatter` strips ACH/withdrawal noise; Bills “Item:” memo line + Amazon empty-memo hint; Adjust funding-gap rows → shared fund confirmation sheet; `GoalStatus.ynabCategoryID`; brighter `muted` + `supportingFont`; Age of Money icon + text (not color-only); system color scheme (removed forced dark); `FundCategoryConfirmationSheet` in Xcode target; tests for payee + goals | See `portfolio/ynab-clarity-ios/CHANGELOG.md` |
