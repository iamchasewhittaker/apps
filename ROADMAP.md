...
|------|-----|---------|----------------|-------|
| 2026-04-13 | Clarity iOS suite | — | **iOS theme alignment:** `ClarityPalette.swift` (clarity-ui, covers 5 Clarity apps) + `ClarityTheme.swift` (YNAB Clarity) updated to portfolio BASE token set (`#0f1117` bg, `#161b27` surface, `#1f2937` border, `#f3f4f6` text, `#6b7280` muted); semantic/accent colors unchanged. See `docs/design/IOS_THEME_ALIGNMENT_HANDOFF.md`. | 2 files, all 6 iOS apps now aligned |
| 2026-04-13 | All 4 web apps | — | **Portfolio theme alignment:** unified Job Search HQ, Wellness Tracker, App Forge, and Knowledge Base onto shared BASE token set (`#0f1117` bg, `#161b27` surface, `#1f2937` border, `#f3f4f6` text, DM Sans font). Each app keeps its own accent color. See `docs/design/PORTFOLIO_WEB_THEME_HANDOFF.md`. | `~109` color swaps + font across 15 files |
| 2026-04-13 | Job Search HQ | v8.5+ | **Wave 2 #1 — Company Intel View:** "By Company" toggle on ContactsTab; company rows expand to show ContactCards; warm lead badges (contact with no app → amber, click to pre-fill AppModal); ghost rows (active app with no contacts → LinkedIn search link); pure computed grouping, no data changes | `portfolio/job-search-hq/` |
| 2026-04-13 | Job Search HQ | v8.5 | **Wave 1 recruiter improvements:** action queue on Focus tab (overdue tasks, stale contacts, prep alerts), next step due dates + type enum on apps, urgency badges on pipeline cards, URL paste quick-capture (AI-assisted JD extraction), outreach scenario chips on AI Tools. New logo: "JOB / HQ" mark, favicon.svg, PWA icons. App Forge + Knowledge Base logos generated. Logo spec + handoff docs in `docs/design/`. | `portfolio/job-search-hq/` |
...
| 2026-04-13 | Knowledge Base | v2.0 | **ARC sidebar + Chrome folder redesign:** two-panel layout (280px collapsible sidebar + scrollable content); nested folder hierarchy (29 categories → 8 parent folders); folder CRUD (create/rename/delete/move); smart folders (Important, Recent); favorites shelf; home view cards; search with breadcrumbs; keyboard shortcuts; mobile overlay; component extraction (6 files: Sidebar, FolderTree, BookmarkRow, BookmarkList, AddEditForm + App shell); SEED_VERSION 5 migration. | `portfolio/knowledge-base/` |
| 2026-04-13 | Knowledge Base | v1.3 | **Category manager + importance ranking:** inline category panel (reorder/rename/merge/delete); importance field (High/Med/Low/None) with colored badges + filter; storage blob upgraded to `{ bookmarks, categoryOrder }`. | `portfolio/knowledge-base/`; deployed to Vercel |
