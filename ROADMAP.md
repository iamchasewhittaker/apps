...
|------|-----|---------|----------------|-------|
| 2026-04-25 | Fairway (iOS) | v0.1 | **Phase 1 zone migration COMPLETE.** `tools/rewrite-kml-labels.py` (NEW) rewrites all 41 `<name>` tags in the Google Earth KML + emits `docs/heads/RENAME_MAP.md` (idempotent). `PreviewData.zone3()` expanded 5→11 heads (`Z3-S1..Z3-S11`, exact photo counts from manifest: Z3-S1=2, Z3-S2=4, rest=3); `zone4()` renamed "Back Yard" with 12 `Z4-S1..Z4-S12` heads + 3 pre-season problem stubs. `FairwayStore.applyPhase1ZoneMigrationIfNeeded()` (sync, called from `load()`) idempotently renames H3-*→Z3-S{N+6}, appends 6 new Z3 heads, replaces Z4 "East Side" placeholder (UUID preserved). Migration test added. All Swift sources parse-clean. `xcodebuild` blocked by missing iOS 17.2 simulator runtime. | `portfolio/fairway-ios/` · CHANGELOG + HANDOFF + LEARNINGS + ROADMAP updated · 3 new LEARNINGS entries (naming collision, photo count variance, UUID preservation) |
