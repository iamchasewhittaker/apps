...
> Last updated: 2026-04-30 (Clarity Budget Web glass theme alignment with Shipyard) | Maintained by Chase
...
|------|-----|---------|----------------|-------|
| 2026-04-30 | Clarity Budget (web) | v0.4 | **Shipyard glass theme alignment.** Dashboard restyled to match Shipyard's structural language: removed `max-w-lg` constraint (fills page), `StsCard` rewrite from one combined card → 3 independent square stat cards in `grid-cols-1 sm:grid-cols-3` (each `rounded-xl border border-dimmer bg-surface/80 backdrop-blur-sm px-6 py-5 transition-colors hover:border-green/20`), Shipyard-style heading inlined (`text-3xl font-bold text-white leading-tight` + steel subtitle, no display font/green-rule), border radius normalized across 5 cards (`rounded-[20px]`/`rounded-2xl`/`rounded-[14px]` → `rounded-xl`), hover effects added to glass cards, main padding `p-10` → `p-8`, category roles section heading switched to Shipyard's `text-xs uppercase tracking-wider text-steel` pattern. Accent stays green (#3db77a) — `PageHeader` untouched for non-dashboard pages. 8 files modified, zero logic changes. tsc ✅ · dev server clean. | `portfolio/clarity-budget-web/` · CHANGELOG + ROADMAP + HANDOFF + LEARNINGS + STATE updated · plan: `~/.claude/plans/we-need-to-natch-sequential-fox.md` |
| 2026-04-30 | Job Search HQ | v8.18 | **Glass redesign: 3-font system + 80% surface glass.** Instrument Sans (body) / Big Shoulders Display (display headlines + stat numbers) / DM Mono (uppercase labels + monospace) across all pages. 4 token changes in `src/tokens.js` (`card` → `rgba(14,26,62,0.80)`, `cardSubtle` → `rgba(14,26,62,0.50)`, `border` → `#1A2A50`, `modalBg` → `rgba(14,26,62,0.92)`) cascade to 40+ usages. 11 files with direct font edits: `constants.js`, `index.css`, `App.jsx`, `FocusTab`, `AITab`, `ContactsTab`, `OfferModal`, `OfferCompareView`, `OutreachTimeline`, `ProfileModal`. letterSpacing normalized to `"0.08em"` across all label styles. | `portfolio/job-search-hq/` · deployed to Vercel |
| 2026-04-30 | Gmail Forge | v0.3 | **Newsletter archiving flip + daily email report.** Newsletters now auto-archive (label + skip inbox) instead of staying visible. `shouldSkipArchive_()` in `auto-sort.gs` — Newsletter line removed. `gmail-filters.xml` — `shouldArchive` added to all 24 Newsletter entries. Apps Script redeployed via clasp. Daily report run: 66 emails (31 inbox / 35 archived). 5 inbox leakers identified: `capacities.io`, `hi.extra.email`, `updates.linear.app`, `james@toybook.com`, GitHub CI → Security mislabel. Logged in `roadmap/inbox-leakers.md`. | `portfolio/gmail-forge/` · CHANGELOG + HANDOFF + LEARNINGS + ROADMAP updated · re-import `gmail-filters.xml` to activate XML layer |
| 2026-04-30 | Job Search HQ | v8.18 | **Glass-card token sweep.** Zero raw hex/rgba outside `src/tokens.js`. All 21 files tokenized (`constants.js`, `App.jsx`, `ErrorBoundary.jsx`, 5 tabs, 10+ components). `docs/BRANDING.md` updated to Command Blue actuals with token-name column + Shipyard alignment. Build clean 186.87 kB. | `portfolio/job-search-hq/` · deployed to Vercel |
| 2026-04-30 | Shipyard | v0.1 | **Sidebar emojis + WIP glass card grid.** Sidebar Fleet nav swapped from Unicode glyphs to real emojis (🚢🔨🏆). `ShipCard` extracted to shared component with `highlight` prop. Dashboard trimmed to summary: StatsBar → panels → 3 Recently Active glass cards → scoreboard/wins. WIP page rebuilt with 3-col `ShipCard` grid + sort controls (priority/last updated/money/MVP step/name); drag-to-reorder removed; `QueueRow.tsx` deleted. | `portfolio/shipyard/` · deployed via `git push origin main` |
| 2026-04-30 | Shipyard | v0.1 | **Sailboat logo + full page consistency pass.** Gold rounded-rect logo with dark navy sailboat icon applied to `LogoIcon.tsx` + `icon.tsx`. All pages updated to glass-card design language: `bg-surface/80 backdrop-blur-sm border border-dimmer rounded-2xl`. Settings + Linear pages now use `ModeHeading`. Ship detail heading upgraded to display font. Zero old tokens (`text-accent`, `border-border`, `bg-card`, `text-foreground`, `text-muted`) remaining across 25 files. | `portfolio/shipyard/` · deployed via `git push origin main` |
| 2026-04-29 | Shipyard | v0.1 | **Readability redesign + three-mode theme.** Full structural dashboard overhaul matching `design/mockup-layout.html` (Option D+ Trustworthy Blue). Three-mode theme: Regular (primary) / Nautical / Roller Coaster Tycoon (RCT). New components: `DashboardHeading`, `ActiveShipPanel`, `ReviewsDuePanel`. Simplified: sidebar nav (grouped FLEET/WORK with icons), `StatsBar` (4 cards, theme-aware), filter bar (type-only chips + count badges + sort), `ShipCard` (name/type/step/status-dot/compliance only). Deleted orphaned `ShipCard.tsx`, `FilterBar.tsx`, `WipBanner.tsx`. History rewrite: Linear API key scrubbed from checkpoint commit via non-interactive rebase. | `portfolio/shipyard/` · CHANGELOG + HANDOFF + LEARNINGS updated · force-push to origin/main pending user auth |
| 2026-04-29 | Job Search HQ | v8.18 | **Command Blue redesign mockup + HQ Monogram logo.** `design/mockup-focus.html` — Focus tab with Shipyard-style readability upgrades (glass cards, 240px sidebar nav, 14px text floor, 44px touch targets, `backdrop-blur` surfaces) adapted to JSHQ's blue `#3b82f6` accent. `design/logo-concepts.html` — 6 SVG logo concepts at 3 sizes; HQ Monogram selected (bold "HQ" filling the icon, DM Sans 800). Design evaluation only — no app code changes. | `portfolio/job-search-hq/design/` · CHANGELOG + HANDOFF + LEARNINGS + ROADMAP updated |
...
| 2026-04-28 | Gmail Forge × Job Search HQ | v0.3 / v8.18 | **Cross-app alignment.** Gmail Forge: `rules.gs` JobSearch rule gains `subjectPatterns` (interview/availability/phone screen/time to chat/schedule a call) — catches recruiter outreach from non-ATS senders; `auto-sort.gs` `matchRules_()` accepts `subject` and runs the regex check after domain/address/toAlias; Gemini system prompt now lists `JobSearch` (was missing — AI couldn't pick it); `normalizeLabel_` accepts `jobsearch`/`job search`/`job-search`; new `healthCheck_jobSearch_()` diagnostic logs trigger + label + thread count + sample unlabeled. `gmail-filters.xml` gains 3 entries (ashbyhq.com, linkedin.com full domain, e.linkedin.com) — 70 → 73. JSHQ: `InboxPanel.jsx` `SetupGuide()` opens with a green "Using Gmail Forge?" callout pointing at `healthCheck_jobSearch_()` (manual filter steps stay for non-Forge users). Build clean (+162 B). **Why:** JSHQ queries Gmail with `labelIds=<JobSearch>` (not `from:`), so Forge must apply the label — recruiters using personal email previously slipped through. | `portfolio/gmail-forge/` + `portfolio/job-search-hq/` · CHANGELOG/HANDOFF/LEARNINGS updated for both apps · plan: `~/.claude/plans/we-need-to-update-fancy-quilt.md` · **deploy:** `cd apps-script && npx clasp push --force` then re-import `gmail-filters.xml` |
| 2026-04-28 | Wellness Tracker (iOS) | Phase 2 shell | **Phase 2 #6 — live timer + active-session controls.** New `Features/Time/TimeCategories.swift` (11 categories ported from web), `WellnessStore.startCategorySession` / `stopActiveSession` / `activeTimeSession` (one running session, persisted in `timeData.active`, day-rollover guard, scripture streak fires on stop). `TimeTabView` rewritten: ticking active card (HH:MM:SS, monospaced digits, DST-midnight guard mirrors `TimeTrackerTab.jsx:181-184`), 2-col category grid (start / switch-to), Today's log unchanged. Retroactive `addTimeSession` API kept. 5 new tests in `WellnessBlobTests`. Phase 2 now 6/6. Also confirmed iOS palette parity is complete (shipped 2026-04-13 commit `91a58f6`); stale roadmap entries ticked + `docs/design/IOS_THEME_ALIGNMENT_HANDOFF.md` marked complete. | `portfolio/wellness-tracker-ios/` · CHANGELOG + ROADMAP + HANDOFF + LEARNINGS updated · plan: `~/.claude/plans/what-s-the-latest-with-breezy-dove.md` |
| 2026-04-27 | Fairway (iOS) | v0.2 | **Overview tab shipped.** New first tab answers "what does the lawn need today?" — `WeatherCard` (Open-Meteo, current + 7-day + last-7-day rain + soil temp), `QuickLogRow` (Mow/Water/Fert/Observation), priority-sorted `AlertsSection`, `PreEmergentSoilTempCard` (Swift Charts `LineMark` + `RuleMark` at 55°F), `PreEmergentCountdownCard`, `AuditProgressCard` (hidden at 100%), `RachioStatusCard`, `ScheduleSanityCard`, `MowStreakCard`. 6 new files (`WeatherSnapshot.swift`, `WeatherAPI.swift`, `OverviewDerived.swift`, `OverviewView.swift`, `WeatherSnapshotTests.swift`, `OverviewDerivedTests.swift`). 58 tests passing (up from 40). **Why Open-Meteo over WeatherKit:** project builds unsigned (`CODE_SIGNING_ALLOWED=NO`), no entitlement available; Open-Meteo is free + HTTPS-only + returns soil temp at 6cm. | `portfolio/fairway-ios/` · CHANGELOG + HANDOFF + LEARNINGS + ROADMAP updated · plan: `we-need-to-make-cryptic-hellman.md` |
| 2026-04-27 | Clarity Command (iOS + web) | v0.2 / v1.0 | **Phase 2 cross-app scoreboard live on iOS.** Discovered web Phase 2 was already coded but dormant (Job Search HQ pushes `job-search-daily` daily — verified via direct Supabase query showing latest row 2026-04-27). Created local `.env` files for clarity-command/job-search-hq/wellness-tracker so dev sync now fires. Built iOS parity: new `Models/CrossAppDaily.swift` (`JobSearchDaily` + `WellnessDaily` with `decodeIfPresent`), `CommandStore` gains `jobSearchDaily`/`wellnessDaily` Observable properties, `CommandCloudSync.pullCrossAppSummaries` runs two REST GETs in parallel via `async let` (RLS already user-scoped, no schema change), new `LiveAppDataView` on Scoreboard mirrors web `LiveAppData` panel with VoiceOver-friendly combined accessibility labels, `ClarityCommandApp.task` auto-pulls on launch. `xcodebuild build` clean for iOS Simulator. | `portfolio/clarity-command/` + `portfolio/clarity-command-ios/` · all docs (HANDOFF/CHANGELOG/ROADMAP/LEARNINGS) updated · plan: `~/.claude/plans/what-s-the-next-step-encapsulated-kernighan.md` |
| 2026-04-27 | Portfolio-wide | — | **Branch consolidation onto `main`.** Long-running `feat/job-search-v8.16` branch (the de-facto trunk through 2026-04-26) plus `feature/ai-categorize`, `chore/archive-rtt-web-rebrand-ios`, and `feat/rtt-ios-v1-simplify` were all verified as fully contained in main (each `ahead-of-main=0`). Pushed pending `27e3c4d` clarity-budget-web doc commit to `origin/main`, then deleted the four stale local branches. Updated dev-workflow memory + per-app HANDOFFs (job-search-hq, clarity-budget-web) so the State tables reflect `main` as trunk. | `git branch -d feat/job-search-v8.16 feature/ai-categorize chore/archive-rtt-web-rebrand-ios feat/rtt-ios-v1-simplify` |
| 2026-04-26 | Shipyard | v0.1 | **Analytics & Themes page overhaul.** Heading now mode-aware via new `analyticsHeading` label key (`"Analytics & Themes"` / `"Charts & Constellations"`). Plain/regular mode is now the default — `DEFAULT_MODE` flipped in `theme-mode.ts`, `readMode()` guard inverted, `useMode()` fallback updated. Scanner auto-populates Common Inputs (9 services) + Cross-App Patterns (8 stacks) by reading each app's `package.json`, `CLAUDE.md`, and Vercel status — 15 rows inserted on first run; nightly cron keeps them fresh. Portfolio Thesis gets an inline editor (server action + client component with ⌘+Enter save). | `portfolio/shipyard/` · CHANGELOG + HANDOFF + ROADMAP + LEARNINGS updated |
| 2026-04-26 | Fairway (iOS) | v0.1 | **Build + spot-check sweep on iPhone 15 sim — clean baseline.** `xcodebuild build` clean, `xcodebuild test` 40/40 passing (was 37 in prior HANDOFF — 3 net-new since session 4). Fresh sim install verified end-to-end via plist blob extraction: property `345 E 170 N, Vineyard, UT` at `40.3004/-111.7456`; Z1=0 heads, Z2=18 + mixed-precip High problem, Z3=11 (no legacy H3- heads — Phase 1 zone migration applied), Z4=12 Z4-S* heads (replaces H4 placeholders); IFA inventory 7.2 lb + dial 3.5 + 2026-04-23 usage entry; `maintenanceTasks`=20 with 4+ recovery items; `fertApplications`=1 (17.8 lb / zones [2,3,4]). Z2-S5 seed correctly says "empty nozzle slot, dirt-packed" (revised from prior MP Rotator claim). HeadPinEditor wiring verified at HeadDetailView.swift:34 (compile-clean, field UX still pending). Two new LEARNINGS entries: HANDOFF-vs-blob field-name drift + plist-extraction recipe for sim verification. | `portfolio/fairway-ios/` · CHANGELOG + HANDOFF + LEARNINGS updated · plan: `whats-next-for-fairway-unified-otter.md` |
| 2026-04-26 | Job Search HQ | v8.17 | **Daily Flow Option E — Morning Launchpad.** Soft-gated 3-stage daily flow at the top of the Focus tab wires Discover (Option B) → Apply (Options A + C) → Outreach (Option D) into one ~80-min container. New `getLaunchpadProgress(applications, dailyActions, now)` derives all stage state from existing data — no new schema. Sunday returns rest mode. Stage 3 `OutreachSprint` adds a `✓ Mark Sent` button (logs `dailyAction("outreach", ...)`) so the 3/day floor is reachable inside the launchpad; sent state persists per-day via `chase_js_outreach_sent_${date}` localStorage. Bonus: `TodaysQueue.handleMarkApplied` now logs `dailyAction("application", ...)` with idempotency guard so Stage 2 progress is reachable without the wizard. Build clean (+2.0 kB gzipped, 176.34 → 176.38 kB). End-to-end verified via preview (Sunday rest live, weekday flow via temp bypass + AppModal save). | `portfolio/job-search-hq/` · CHANGELOG + ROADMAP + LEARNINGS + HANDOFF + BRANDING + repo CLAUDE.md updated · plan: `~/.claude/plans/ready-for-email-linkedin-notification-linked-stream.md` |
| 2026-04-25 | Clarity Budget (web) | v0.4 | **v1 Redesign Step 3 — Privacy.com integration.** New `lib/privacy/{types,client,sync}.ts`. `createPrivacyClient(token)` factory with `Authorization: api-key <token>` (Privacy's scheme — not Bearer); 50/page pagination; `listCards()` + `listTransactions({ begin? })`. `pullCards(userId)` + `pullTransactions(userId, since?)` decrypt the user's `privacy_token_*` from `clarity_budget_credentials`, upsert in chunks of 500, **preserve user-owned columns** (`linked_payee_id`, `matched_ynab_txn_id`) by pre-fetching existing rows and re-applying their values, stamp `clarity_budget_sync_state` (`source='privacy'`) with success/error, and emit `clarity_budget_audit_log` rows (`actor='system'`). Idempotent. `pnpm exec tsc --noEmit` + `pnpm lint` + `pnpm build` all clean (route output unchanged from Step 2). Manual smoke pending (needs `SUPABASE_SERVICE_ROLE_KEY` + Privacy.com token). | `portfolio/clarity-budget-web/` · Step 4 (reconcile logic + first vitest suite) up next · Money Companion still deferred until redesign ships |
| 2026-04-25 | Fairway (iOS) | v0.1 | **Zone-menu refinements (session 4).** `ProblemData.actions: [String]` (Codable `decodeIfPresent`, default `[]`); `ProblemAreaView` Open section renders bullet actions, new "More — Shopping List" section. `FairwayStore.recommendedNozzleShoppingList(for:)` derivation (short-radius MP800 SR vs. zone-target buckets). `HeadInventoryView` Z2 sectioned into Park Strip / Main Yard. `ScheduleView` Read-only mirror banner + "?" info popovers per parameter + Recent-fertilizer card (water-in window status). `MapTabView` per-zone convex-hull polygon overlay (Andrew's monotone-chain) — disambiguates Z3 NW corner from Z4. Pre-season speculative problems dropped across all zones. New rule #10 in `Fairway/CLAUDE.md`: heads = irrigation hardware only, no per-head fert. `testProblemActionsBackwardCompat` + `testNozzleShoppingListForZ2` added. | `portfolio/fairway-ios/` · CHANGELOG + HANDOFF + LEARNINGS updated · plan: `in-the-zone-menu-memoized-breeze.md` |
| 2026-04-25 | Unnamed (iOS) | v0.1.2 | **UX parity with web.** Inbox edit/delete (`InboxRowView` — view/edit/confirmDelete modes, 44pt pencil + trash, 3s confirm), sort lane help sheet (`LaneHelpSheet` via ⓘ button — summary/examples/rule, "Sort into [Lane]" CTA), check clarification (`LockedLanesHeader` showing today's locked lanes as chips in both `CheckFormView` + `CheckDoneView`; Q1/Q2 reworded with helper text). `Lanes.swift` gains `summary`, `examples[]`, `rule`; `Lane: Identifiable`. `AppStore` gains `updateItemText` + `deleteItem`. New `LockedLanesHeader.swift` registered in pbxproj. Build clean, installed + launched on device. | `portfolio/unnamed-ios/` · all docs updated |
| 2026-04-25 | Unnamed (web) | v0.1 | **UX clarifications from Phase 1 real use.** Inbox edit/delete (visible pencil + trash, confirm-delete), sort lane help sheet (ⓘ → bottom sheet with LANE_HELP), check clarification (locked-lane chips at top, reworded Q1/Q2). 5 src files + docs committed. | `portfolio/unnamed/` · CHANGELOG + HANDOFF + LEARNINGS updated |
| 2026-04-25 | Fairway (iOS) | v0.1 | **Zone migration verified + installed on device. 3 production Codable bugs fixed.** All tests pass (EXIT:0) including `test_phase1ZoneMigration_renamesH3toZ3S_andSeedsZ4BackYard`. Fixed `PropertySettingsView` Swift concurrency (`@MainActor` on struct); `HeadData` backward-compat decoder (synthesized `init(from:)` required absent JSON keys, crashing existing installs); `FairwayBlob` same Codable issue for newer array fields. All decoders moved to `extension` (not struct body) to preserve memberwise init. App built for device, installed via `devicectl`, smoke test passed: Zone 3=11 heads Z3-S1..Z3-S11, Zone 4="Back Yard"+12. **iOS Codable Safety Rules added to root CLAUDE.md** (applies to all portfolio iOS apps). | `portfolio/fairway-ios/` · 2 new LEARNINGS entries (Codable decodeIfPresent rule, pipe exit code masking) · root CLAUDE.md updated with iOS Codable Safety Rules |
| 2026-04-25 | Fairway (iOS) | v0.1 | **Phase 1 zone migration COMPLETE (Swift).** `tools/rewrite-kml-labels.py` (NEW) rewrites all 41 `<name>` tags in the Google Earth KML + emits `docs/heads/RENAME_MAP.md` (idempotent). `PreviewData.zone3()` expanded 5→11 heads (`Z3-S1..Z3-S11`); `zone4()` renamed "Back Yard" with 12 `Z4-S1..Z4-S12` heads + 3 pre-season problem stubs. `FairwayStore.applyPhase1ZoneMigrationIfNeeded()` (sync, called from `load()`) idempotently renames H3-*→Z3-S{N+6}, appends 6 new Z3 heads, replaces Z4 "East Side" placeholder (UUID preserved). Migration test added. | `portfolio/fairway-ios/` · 3 new LEARNINGS entries (naming collision, photo count variance, UUID preservation) |
| 2026-04-25 | Unnamed (web) | v0.1 | **Deployed to Vercel.** Production: https://unnamed-gold.vercel.app. New Vercel project `iamchasewhittakers-projects/unnamed`, linked + git-connected to `iamchasewhittaker/apps`. Project settings via API: `framework: nextjs`, `rootDirectory: portfolio/unnamed`, `ssoProtection: null`. CLI deploy ran clean from `portfolio/unnamed/` after temporarily clearing `rootDirectory` (avoids Vercel CLI's path-doubling when cwd already equals rootDirectory). All 5 routes serve 200; `/manifest.json` content matches local; PWA icons (192/512) present. `scripts/vercel-check-git` reports unnamed connected. Phase 1 7-day clock starts. **No new features.** | `portfolio/unnamed/` · CLAUDE.md row + CHANGELOG + HANDOFF + ROADMAP + LEARNINGS updated · Shipyard sync re-run |
| 2026-04-24 | Unnamed (web) | v0.1 | **Web/iOS parity reconciled.** Three divergences fixed: (1) Sort flow gained a Skip button (matches iOS / spec "pick a lane or skip"); (2) `skipItem` rewritten to cycle the item to the end of `state.items` (was marking `status: "skipped"`) — matches iOS `skipItem` / `skipActiveItem`; (3) FocusView `allItems` now reads from `state.items` directly (was `lanes.flatMap(...)` which broke the visible Skip cycle). All 5 flows verified end-to-end via dev preview. `pnpm build` clean (Next.js 16 Turbopack, 9 routes). `ItemStatus = "skipped"` left in the type for iOS parity. **Vercel deploy pending in next session.** | `portfolio/unnamed/` · iOS chosen as source of truth (shipped second, hand-tuned on device) · 3 source files + CHANGELOG + HANDOFF + LEARNINGS + ROADMAP touched |
| 2026-04-24 | Fairway (iOS) | v0.1 | **KML sprinkler ingestion source edits landed (session 2).** `PreviewData.phase0Z2Heads()` rewritten to return 18 heads (was 6): Z2-S1..S6 carry lat/lon/photoPaths sourced from `docs/heads/Z2-MATCH-1st/`..`Z2-MATCH-6th/`; Z2-S7..S18 appended (12 front-yard heads, generic Hunter Pro-Spray bodies, KML pin name in `notes`, `isConfirmed: false`); Z2-S1 corrected to "Rain Bird VAN yellow" `radiusFeet: 4` (was "Brass adjustable") from photo-1 evidence; Z2-S5 corrected to `isConfirmed: false`, "TBD — nozzle slot empty in photo (dirt-packed)" — photo-1 of `Z2-MATCH-5th/` showed empty Hunter Pro-Spray slot, overriding the seed's "MP Rotator confirmed" claim. `phase0Z2MixedPrecipProblem()` description rewritten to drop the S5-MP premise (now reframes around 3 nozzle families: VAN yellow / 1555 fixed spray / empty slot). `zone3()` H3-1..H3-5 each gained `latitude`/`longitude`/`photoPaths` (final names `heads/H3-N/photo-N.jpg`) + KML pin name in notes. `FairwayBlobTests.swift` +2 tests: `testHeadDataPhotoPathsRoundTrip` (encode/decode with photoPaths), `testZone2HasEighteenSeededHeads` (count + first/last labels + all heads have coordinates and photoPaths). Park-strip photo dirs intentionally NOT renamed `Z2-MATCH-1st/` → `Z2-S1/` yet — provisional matching needs Chase field confirmation, especially Z2-S5 anomaly. | `portfolio/fairway-ios/` · `xcodebuild` build/test still blocked (iOS 17.2 simulator runtime missing); source verified by inspection · pending: install runtime + run tests + generate `PHOTO_AUDIT.md` + `COVERAGE_ANALYSIS.md` + `PROPERTY_PLACEMENT.md` |
| 2026-04-24 | Fairway (iOS) | v0.1 | **Rachio API integration (v1, read-only) shipped.** New `Services/RachioKeychain.swift` (Security framework, `kSecAttrAccessibleWhenUnlockedThisDeviceOnly`), `Services/RachioAPI.swift` (async URLSession, typed `RachioError`), `Services/RachioDTOs.swift` (wire types kept separate from persisted models), `Models/RachioState.swift` (zones + schedule rules + flex rules + events + zone links). New `Views/RachioSettingsView.swift` (SecureField → Verify & Connect → device picker → zone-link pickers → Disconnect) and `Views/RachioHistoryView.swift` (events grouped by day). `FairwayBlob` gains optional `rachio: RachioState?` (legacy blobs decode unchanged). `FairwayStore` gains `verifyRachioToken`, `completeRachioConnection`, `syncRachio`, `disconnectRachio`, `setRachioZoneLink`; auto-links Fairway Z1–Z4 to matching Rachio zone numbers; 90-day initial history, 1-day overlap on incremental sync, dedup by id, 500-event cap. `ScheduleView` gains read-only "Rachio says" mirror card. 7 new `RachioDecodeTests` (DTO decode, zone-link lookup, legacy-blob migration). 7 files registered under FW037–FW03D. Token never hardcoded — entered via `SecureField` and written to Keychain only after `/person/info` succeeds. | `portfolio/fairway-ios/` · Swift typecheck clean · `xcodebuild` verification deferred (iOS 17.2 simulator runtime not installed on this machine) |
| 2026-04-22 | Idea Kitchen | v0.3 | **Artifact reliability + paste fallback.** Diagnosed a retroactive-docs run where claude.ai emitted 6 fenced code blocks in chat instead of artifact panels — root cause was `<antArtifact>` XML shown as code examples in STEP 6 / STEP 6F (sometimes echoed verbatim) plus latent "All six STEP 6 **blocks**" wording in 40 SESSION_START templates. Fixes: top-of-prompt "artifacts are artifacts, not code blocks" rule; STEP 6 + STEP 6F rewritten using imperative filename language; both installers gained `--paste <file>` / `--paste-clipboard` with `--- FILE: <name> ---` delimiter splitter (BSD awk compatible, preamble discarded); portfolio-wide sweep normalized "blocks" → "artifacts (downloadable panels, not code blocks in chat)" across 40 templates + `SESSION_START_EXISTING_APP.md` + `SESSION_START_NEW_APP_EXAMPLE.md`; BUILD_GUIDE §4/§4b/§11 updated. Splitter + error-path unit-tested. | `portfolio/idea-kitchen/` · re-upload prompt + 40 swept templates to live Claude Project · project + feature modes both patched |
| 2026-04-22 | Idea Kitchen | v0.2 | **Feature mode shipped.** Second entry branch to the Claude Project — "add X to `<target-app>`" phrasing routes into feature mode. STEPs 0F → 7F: target intake, cross-portfolio duplication scan (4 checks, `EXTEND_TARGET \| EXTRACT_SHARED \| NEW_APP \| KILL`), 4-layer competitor research (feature matrix + UX teardown + review mining + technical approach, `KILL \| DIFFERENTIATE \| PROCEED` with 3–5 levers), Feature Brief + PRD + Design + Impl Plan, 4-artifact generation. 4 new templates (`FEATURE_{BRIEF,PRD,DESIGN,IMPL_PLAN}_TEMPLATE.md`). `scripts/install-feature-docs` — idempotent installer wires ROADMAP/CHANGELOG/LEARNINGS/HANDOFF + Obsidian feature hub + parent-hub link in one run. BUILD_GUIDE section 4b added with Fairway sprinkler-map worked walkthrough. | `portfolio/idea-kitchen/` · project mode preserved unchanged · dogfood pending (Fairway sprinkler-map) |
| 2026-04-21 | Idea Kitchen | v0.1 | **Scaffolded `portfolio/idea-kitchen/` (docs-only app).** Claude Project system prompt (10-step flow) + master build guide + 3 templates (SHOWCASE, Knowledge Manifest, SESSION_START example) + dogfooded Phase 1/2/2.5/3 docs. Seeded `identity/patterns.md`. Added Portfolio Overview + Shipyard metadata rows. Turns raw ideas → shipped portfolio apps via one Claude Project + one Claude Code handoff. | 16 files created in `portfolio/idea-kitchen/` · [docs/BUILD_GUIDE.md](portfolio/idea-kitchen/docs/BUILD_GUIDE.md) is the entry |
| 2026-04-20 | RollerTask Tycoon (web) | v1.0 | **Vercel build fix:** repeated failed production deployment emails traced to `vite` in `devDependencies` — Vercel's `NODE_ENV=production` install skips devDeps; stale build cache compounded the issue by bypassing install entirely. Fix: moved `vite` → `dependencies`, added `installCommand: "npm install --include=dev"` to `vercel.json`, force-deployed to bust poisoned cache. All 6 Vercel projects now green. `LEARNINGS.md` created. | `portfolio/roller-task-tycoon/` · commits `d08eb90` + `8ee6d43` |
| 2026-04-20 | Job Search HQ | v8.12 | **Wave 4 #5:** Email forward parsing — `parseRecruiterEmail()` regex parser in `constants.js` (name from From header + signature, company from email domain, role from "I'm a X at" pattern, job title from phrase patterns, LinkedIn URL, ATS/job posting URL); 📧 Email Parse sub-tab in Apply Tools; editable extracted fields → pre-filled ContactModal or AppModal. Build clean (+1.64 kB). | `portfolio/job-search-hq/` · 6/7 fields extracted in Stripe recruiter email smoke test |
| 2026-04-20 | Job Search HQ | v8.11 | **Wave 4 #4:** Offer comparison side-by-side — `offerDetails` object on every app (base/bonus/bonus type/sign-on/equity/PTO/dates/location/remote/benefits/notes), new `OfferModal` editor with live "Total comp estimate" preview (`base + bonus + equity + signOn/4`), new `OfferCompareView` table with best-in-column green highlights + single-offer card fallback + empty state. 💰 Offer button added to AppCard when stage=Offer. New collapsible "💰 Offer comparison (N)" section above Win/Loss Analytics on Pipeline (auto-opens when ≥2 offers). Root `CLAUDE.md` Doc Auto-Update Rule extended with steps 8 (Linear heartbeat) + 9 (Shipyard sync). | `portfolio/job-search-hq/` · build +3.37 kB clean · backwards-compatible via `normalizeApplication` |
| 2026-04-20 | Job Search HQ | v8.10 | **Wave 4 #3:** Per-contact outreach cadence timeline — `outreachLog[]` history on every contact, `OutreachTimeline` vertical list on ContactCard (colored type dot + date + type + method + notes; default 4 visible, "Show all" toggle, empty state). `OUTREACH_EVENT_TYPES` + `OUTREACH_METHODS` + `blankOutreachEntry` + `normalizeOutreachLog` + `normalizeContact` in `constants.js`; migration seeds legacy `outreachDate`/`outreachStatus` on load. ContactModal "Outreach Log" entry form + delete. Card status-dropdown change appends an entry automatically. | `portfolio/job-search-hq/` · verified modal-add + quick-log in browser · build clean |
| 2026-04-20 | Clarity Budget (web) | v0.4 | **Session 2 — spending visibility:** `lib/aggregations.ts` (flatten split tx → `SpendLine[]`, group by category/payee/week/account), `lib/filterState.ts` (`FilterState` + URL-persisted `useUrlFilterState` hook), `components/TransactionFilters.tsx` (collapsible, count badge, date/payee/amount/multi-select category+account), `components/SpendingBreakdown.tsx` (3 tabs), `components/TransactionList.tsx` (sortable + role chips + paginated). `HomeDashboard` inline memos removed; `app/page.tsx` wrapped in `<Suspense>` for `useSearchParams`. Lint + build clean. | `portfolio/clarity-budget-web/` · WHI-72 ready to close |
...
| 2026-04-20 | Knowledge Base | v2.1.1 | **Vercel redeploy + seed maintenance:** re-connected to Vercel (knowledge-base-hazel-iota.vercel.app) + apps.git auto-deploy; fixed 4 My Projects URLs; added Vercel + Supabase folders (8 links each); 4 Git links; Alias Ledger + Clarity Budget Web in My Projects; SEED_VERSION 11 | `portfolio/knowledge-base/` |
...
| 2026-04-16 | Gmail Forge | — | **Rename + archived report:** renamed from "Inbox Zero" → "Gmail Forge" (~130 replacements, 48 files, 6 projects); daily report extended to include `🗄️ Auto-archived today` section + totals line so all received mail is visible | Off-filesystem: rename Apps Script project + update Spend Radar Script Properties manually |
| 2026-04-13 | Gmail Forge | — | **Phase 3 kickoff:** Apps Script auto-sorter (`auto-sort.gs` + `rules.gs`) with Gemini classification + Rules-only fallback + Sheets logging; Chrome extension (`extension/`) with label tab bar, Sort button, settings popup, dark mode | Pending deployment to script.google.com + Chrome |
...
| 2026-04-12 | Gmail Forge | — | **New project: Gmail Forge** — Gmail XML filter system (60 filters); Security label; ZipRecruiter domain whitelist; LinkedIn messages unarchived for job search; Spend Clarity integration designed; CLAUDE.md + HANDOFF.md + LEARNINGS.md + CHANGELOG.md created; moved to `portfolio/gmail-forge/` | Free tool, no paid services; daily Claude report workflow |
...
| 2026-04-04 | Portfolio | — | **Legacy monorepo under Documents** (former **`apps`** + **`Projects`** trees) → **`~/Documents/_archive_legacy_monorepo_20260404/`**; added **`docs/GLOSSARY.md`**; refreshed **DOCUMENTS_GIT_ARCHIVE_REMOVED** + **MONOREPO_MIGRATION** | Active code only under `~/Developer/chase/portfolio` + `projects/` |
