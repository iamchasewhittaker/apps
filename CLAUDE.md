...
| Clarity Budget (web)        | v0.4          | `chase_budget_web_v1` + `chase_budget_web_tx_v1` (tx cache) + YNAB token key in `localStorage` + Supabase-encrypted `clarity_budget_credentials` (YNAB + Privacy tokens, AES-256-GCM)                                                                                          | clarity-budget-web.vercel.app       | ✅ **v1 Redesign Steps 1–8 done** (2026-04-29) · Step 1 backend · Step 2 auth (SSR + OAuth) · Step 3 Privacy.com · Step 4 reconcile · Step 5 AI categorize · Step 6 /review UI · Step 7 /flags UI · Step 8 /settings Privacy connector + card mapping · **Step 9 next** · Money Companion (Phase 2) deferred · `[portfolio/clarity-budget-web](portfolio/clarity-budget-web)`                                              |
...
| Clarity Command             | v1.0          | `chase_command_v1`                                                                                                           | clarity-command.vercel.app          | ✅ Active · Daily accountability hub · 5-app Scoreboard (Job Search + Wellness + Time + Budget + Growth) · `[portfolio/clarity-command](portfolio/clarity-command)`                                                      |
| Job Search HQ               | v8.18         | `chase_job_search_v1` + Supabase row `app_key='job-search:gmail'` (encrypted refresh token, server-only)                     | job-search-hq.vercel.app            | ✅ Active · v8.18 Gmail Inbox Feed (Focus-tab `InboxPanel` between MorningLaunchpad + TargetCompanyBoard; browser GIS popup OAuth → server-side exchange in `api/gmail/*` → AES-256-GCM-encrypted refresh in Supabase; `src/inbox/classifier.js` buckets recruiter / ats_update / interview_invite / linkedin; review-first queue with one-click Save Contact + App / Bump stage / Set interview + open prep / Dismiss / Snooze; auto-logs wins; read-only — replies stay in Gmail) · v8.17 Daily Flow Option E (Morning Launchpad) · v8.16 Discovery Sprint + Apply Wizard · v8.15 TargetCompanyBoard · v8.14 Today's 5 + Outreach Autopilot · Wave 4 #1–#6 all shipped · Chrome MV3 in `extension/` |
...
| RollerTask Tycoon (iOS)     | v2.0          | SwiftData + `UserDefaults` (`chase_roller_task_tycoon_ios_*`)                                                                | local Xcode                         | 🟡 Local · V2 Game Feel shipped (subtasks, templates, haptics) · [Linear](https://linear.app/whittaker/project/park-checklist-ios-b0d5872be46e)                                                                                                                               |
| RollerTask Tycoon (web PWA) | v1.0          | `chase_roller_task_v1`                                                                                                       | local                               | 🟡 Code-only · Vite PWA · Vercel project removed 2026-04-25 (persistent build failures) · `[portfolio/roller-task-tycoon](portfolio/roller-task-tycoon)`                                                                |
...
| AI Dev Mastery              | v1.1.0        | none (no persistence)                                                                                                        | ai-dev-mastery.vercel.app           | ✅ Active · React CRA course · 15 tracks, 80 modules · orange `#f97316` branding · `[portfolio/ai-dev-mastery](portfolio/ai-dev-mastery)`                                                                                 |
...
| Keep (iOS) | v0.1 | `chase_keep_ios_v1` | local Xcode | 🟡 Phase 1 built · SwiftUI + @Observable · triage + zone flow · BUILD SUCCEEDED · xcodeproj (`KP*`) · `[portfolio/keep-ios](portfolio/keep-ios)` |
...
| Gmail Forge                 | v0.3          | Apps Script Script Properties (`CLASSIFIER_MODE`, `GEMINI_API_KEY`, `SHEET_ID`, `NEWSLETTER_TO_ALIASES`, `TRIGGER_TOKEN`)        | script.google.com                   | ✅ Active · Auto-sort + label engine (5-min trigger, 73 sender rules + 5 JobSearch subject regexes, RULES_ONLY mode); applies `JobSearch` label that powers Job Search HQ's InboxPanel · `[portfolio/gmail-forge](portfolio/gmail-forge)` |
...
| Ash Reader (iOS)            | v0.4          | `ash_reader_ios_` prefix (UserDefaults)                                                                                       | local Xcode                         | 🟡 Local · Phases 5-7 done (iCloud sync, reminders, sharing, streaks) · 41/41 tests · P6 yellow icon · `[portfolio/ash-reader-ios](portfolio/ash-reader-ios)` |
| Unnamed (web)               | v0.1          | `unnamed_v1`                                                                                                                  | unnamed-gold.vercel.app             | ✅ Active · Daily OS for ADHD brains · **Web/iOS parity reconciled (2026-04-24)** · Next.js + Tailwind 4 + localStorage · **Deployed 2026-04-25** · Phase 1 7-day clock running · `[portfolio/unnamed](portfolio/unnamed)` |
...
| Fairway (iOS)               | v0.2          | `chase_fairway_ios_v1` + Keychain `com.chasewhittaker.Fairway`                                                                | local Xcode                         | 🟡 Local · SwiftUI + @Observable · Lawn OS: 4 zones, irrigation, fertilizer plan, soil test, spreader calc, shrub beds · **Rachio v1 read-only** · **Overview tab v0.2** (Open-Meteo weather + soil-temp chart + alerts + audit progress + schedule sanity + mow streak · 62 tests) · Phase 2 Z2 sub-zones · Augusta green palette · xcodeproj (`FW*`) · `[portfolio/fairway-ios](portfolio/fairway-ios)` |
| Fresh Track                 | v0.1          | `chase_fresh_track_v1`                                                                                                        | fresh-track-sigma.vercel.app        | ✅ Active · Grocery freshness tracker · FoodKeeper autocomplete + fridge/freezer tabs + urgency badges + printable fridge list · `[portfolio/fresh-track](portfolio/fresh-track)` |
...
| Idea Kitchen                | v0.3          | n/a (docs-only)                                                                                                               | local                               | ✅ Active · **v0.3 artifact reliability + paste fallback** · prompt hardened (no more `<antArtifact>` XML in examples) · both installers gained `--paste <file>` / `--paste-clipboard` with `--- FILE: <name> ---` splitter · 40 SESSION_START templates swept "blocks" → "artifacts" · two modes: project (6 artifacts) + feature (4 artifacts) · `[portfolio/idea-kitchen](portfolio/idea-kitchen)` |
...
| --- | --- | --- | --- | --- |
| Wellness Tracker | Dashboard | Daily wellness check-in with streaks | [apps](https://github.com/iamchasewhittaker/apps) | [Linear](https://linear.app/whittaker/project/wellness-tracker-36f4fb10e0e7) |
| Wellness Tracker (iOS) | Health & Fitness | Native SwiftUI check-in shell | — | — |
| ClarityUI (Swift pkg) | Developer Tools | Shared SwiftUI components for the Clarity iOS apps | — | [Linear](https://linear.app/whittaker/project/clarityui-24503a4b3258) |
| Clarity Check-in (iOS) | Health & Fitness | Morning check-in + meds tracker | — | [Linear](https://linear.app/whittaker/project/clarity-check-in-ios-a22a5a1b0e6c) |
| Clarity Triage (iOS) | Productivity | Daily capacity, ideas, and wins | — | [Linear](https://linear.app/whittaker/project/clarity-triage-ios-f9092666e165) |
| Clarity Time (iOS) | Productivity | Time sessions + scripture streak | — | [Linear](https://linear.app/whittaker/project/clarity-time-ios-1ac861a8b710) |
| Clarity Budget (iOS) | Finance | YNAB-backed STS month/week/day budget | — | [Linear](https://linear.app/whittaker/project/clarity-budget-ios-22642243fa11) |
...
| Clarity Growth (iOS) | Lifestyle | 7 growth areas with streaks | — | [Linear](https://linear.app/whittaker/project/clarity-growth-ios-390ab290b06d) |
| Clarity Command (iOS) | Productivity | Mission + Scoreboard + Settings — daily accountability | — | [Linear](https://linear.app/whittaker/project/clarity-command-ios-95893cfe530c) |
| Clarity Command | Dashboard | Daily accountability hub on the web | [apps](https://github.com/iamchasewhittaker/apps) | [Linear](https://linear.app/whittaker/project/clarity-command-ab0bbd71be7c) |
...
| App Forge | Tool | Web workbench for building new portfolio apps | [apps](https://github.com/iamchasewhittaker/apps) | [Linear](https://linear.app/whittaker/project/app-forge-64221811d236) |
| Clarity Hub | Dashboard | Unified 5-tab hub (Check-in / Triage / Time / Budget / Growth) | [apps](https://github.com/iamchasewhittaker/apps) | [Linear](https://linear.app/whittaker/project/clarity-hub-3c085a0dd7a2) |
| Funded (iOS) | Finance | YNAB read + assign flow, Keychain-secured | — | [Linear](https://linear.app/whittaker/project/funded-ios-0fa6245bef6a) |
...
| AI Dev Mastery | Content | 13-track React course on AI-assisted dev | [apps](https://github.com/iamchasewhittaker/apps) | [Linear](https://linear.app/whittaker/project/ai-dev-mastery-920c7f60c820) |
| Shortcut Reference | Reference | macOS menu-bar keyboard shortcut viewer | — | [Linear](https://linear.app/whittaker/project/shortcut-reference-5b5280d7288b) |
...
| Spend Clarity | Developer Tools | Python CLI enriching YNAB from Gmail + Privacy.com | — | [Linear](https://linear.app/whittaker/project/spend-clarity-4352a4e7f1c5) |
| Knowledge Base | Content | Personal bookmark + notes hub | [apps](https://github.com/iamchasewhittaker/apps) | [Linear](https://linear.app/whittaker/project/knowledge-base-65f42b7a8807) |
| Keep (iOS) | Lifestyle | One-item-at-a-time triage: keep, donate, or toss — give every kept item a home | — | — |
| Fresh Track | Lifestyle | Grocery freshness tracker — receipt scanning, expiration dates | [apps](https://github.com/iamchasewhittaker/apps) | — |
| Funded Web | Dashboard | Standalone YNAB dashboard | [apps](https://github.com/iamchasewhittaker/apps) | [Linear](https://linear.app/whittaker/project/funded-web-f17432cff6d6) |
| RollerTask Tycoon Web | Tracker | Standalone tasks/points tracker (split from Hub) | [apps](https://github.com/iamchasewhittaker/apps) | [Linear](https://linear.app/whittaker/project/rollertask-tycoon-web-53da1d06226e) |
...
| Shipyard | Dashboard | Fleet command center for the portfolio | [apps](https://github.com/iamchasewhittaker/apps) | [Linear](https://linear.app/whittaker/project/shipyard-2847e4a05666) |
| Shipyard (iOS) | Productivity | Mobile companion to Shipyard | — | — |
| Ash Reader | Tool | Mobile chunker for capture conversation → Ash | [apps](https://github.com/iamchasewhittaker/apps) | [Linear](https://linear.app/whittaker/project/ash-reader-c2d6692cd906) |
| Ash Reader (iOS) | Utilities | Native SwiftUI Ash Reader with themes + actions | — | — |
| Unnamed (web) | Tool | Daily OS for ADHD brains — 7-day rule | [apps](https://github.com/iamchasewhittaker/apps) | [Linear](https://linear.app/whittaker/project/unnamed-9c31c84422f9) |
| Unnamed (iOS) | Productivity | On-device daily OS companion | — | — |
| Fairway (iOS) | Lifestyle | Lawn OS: zones, irrigation, fertilizer plan, soil test, Open-Meteo weather | — | [Linear](https://linear.app/whittaker/project/fairway-ios-967c8e495407) |
| Alias Ledger | Utilities | Hide My Email alias tracker (single-file HTML) | [apps](https://github.com/iamchasewhittaker/apps) | [Linear](https://linear.app/whittaker/project/alias-ledger-401f592bd67c) |
| Gmail Forge | Tool | Apps Script auto-sort + label engine for Gmail (paired with Extra for triage) | — | [Linear](https://linear.app/whittaker/project/gmail-forge-110c46ff126d) |
...
| GMAT Mastery Web | Education | AI-powered GMAT prep with Claude API | [apps](https://github.com/iamchasewhittaker/apps) | [Linear](https://linear.app/whittaker/project/gmat-mastery-web-7599a39fd355) |
| DriveMind | Developer Tools | macOS app for consolidating external drives + NAS | — | [Linear](https://linear.app/whittaker/project/drivemind-130f5ea4bdb0) |
...
> - *Handoffs:* "What would a new agent need to know to pick this up cold?" / "What context did you wish you had at the start of this session?"
> - *Claude mastery:* "What prompt technique saved the most time today?" / "What would you do differently if the context window were half the size?"
> - *Architecture:* "If this app had 100x users tomorrow, what breaks first?" / "What's the simplest thing you could remove without anyone noticing?"
