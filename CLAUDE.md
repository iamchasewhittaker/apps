...
| Clarity Budget (web)        | v0.4          | `chase_budget_web_v1` + `chase_budget_web_tx_v1` (tx cache) + YNAB token key in `localStorage` + Supabase-encrypted `clarity_budget_credentials` (YNAB + Privacy tokens, AES-256-GCM)                                                                                          | clarity-budget-web.vercel.app       | ✅ **v1 Redesign Steps 1–3 done** (2026-04-25) · Step 1 backend (7 `clarity_budget_*` tables + RLS + crypto + `/api/credentials`) · Step 2 auth (SSR cookie middleware + password + GitHub OAuth + app-shell layout) · Step 3 Privacy.com integration (`lib/privacy/{types,client,sync}.ts` — `pullCards` + `pullTransactions` decrypt token, upsert with user-column preservation, stamp `clarity_budget_sync_state` + audit log) · **Step 4 next** (reconcile logic + first vitest suite) · Money Companion (Phase 2) deferred until redesign ships · `[portfolio/clarity-budget-web](portfolio/clarity-budget-web)`                                              |
...
| Gmail Forge | Tool | Apps Script auto-sort + label engine for Gmail (paired with Extra for triage) | — | — |
