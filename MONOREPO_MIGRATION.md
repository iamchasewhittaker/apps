...
- **AI Dev Mastery + Shortcut Reference + Claude Usage Tool → `portfolio/`** (2026-04-14): `ai-dev-mastery` and `shortcut-reference` promoted from `projects/` → `portfolio/` (active). `claude-usage-tool` un-archived from `projects/archive/` → `portfolio/` (reactivated). Full portfolio doc scaffolding (HANDOFF, LEARNINGS, BRANDING, CHANGELOG, ROADMAP per app), Cursor symlinks, root docs updated, Linear projects created/reactivated. AI Dev Mastery added to CI workflow.
...
- **Vercel Root Directory** set (team `iamchasewhittakers-projects`): **`wellness-tracker`** → **`portfolio/wellness-tracker`**, `job-search-hq` → `portfolio/job-search-hq`, `app-forge` → `portfolio/app-forge`; **RollerTask Tycoon** web PWA was retired to `portfolio/archive/roller-task-tycoon` (disable or repoint Vercel if desired); `growth-tracker` (retired) → `portfolio/archive/growth-tracker`. **iOS** RollerTask: `portfolio/roller-task-tycoon-ios` (Xcode, not Vercel). Redeploys should follow from git pushes; confirm in each project’s Deployments tab.
