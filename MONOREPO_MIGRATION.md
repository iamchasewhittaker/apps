...
- **`main` pushed to GitHub** (`iamchasewhittaker/apps`). If `git push` returns HTTP 400, run `git config http.postBuffer 524288000` and retry (large pack). GitHub may warn about files over 50MB (e.g. `projects/claude-usage-tool` GIF) — consider [Git LFS](https://git-lfs.github.com/) later.
