...
| **Focus**      | **Fairway iOS — field walk** (More → Pre-Season Audit, 41 heads). Session 4 zone-menu refinements shipped: Z2 sub-grouping (park strip / main yard), problem actions + nozzle shopping list, schedule explainers (read-only banner + "?" popovers + recent-fert card), map zone-hull overlays. Z4-S1 still highest priority (buried, zero coverage). |
| **Next**       | 1. **Fairway iOS:** field walk — open audit sheet per head, enter arc/radius/nozzle/GPM, tick "Head cleared". 2. **Fairway iOS:** visually verify map zone-hulls separate Z3 NW pins from Z4 cluster on device. 3. **Fairway iOS:** seed actions for Z2 phase0 problem more granularly post-audit. 4. **clarity-budget-web v1 redesign — Step 4** (reconcile logic + first vitest suite); Steps 1–3 done (auth refactor + Privacy.com integration). Money Companion deferred until redesign ships. 5. **Wellness:** split TrackerTab.jsx (WHI-63). |
| **Blockers**   | None — iOS 17.2 simruntime is mounted, device install works. |
| **Last touch** | 2026-04-25 — Fairway iOS session 4: `ProblemData.actions` field + bullet UI; `recommendedNozzleShoppingList`; Z2 Heads sub-grouping; ScheduleView read-only banner + info popovers + recent-fert card; MapTabView convex-hull zone overlays. Build clean. |
