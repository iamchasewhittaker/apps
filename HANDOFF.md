...
| **Focus**      | **Fairway iOS — field walk** (More → Pre-Season Audit, 41 heads). Audit sheet now pre-fills estimates and has a measurement guide (catch-cup GPM test). Map coordinates fixed. App on device. Z4-S1 highest priority (buried, zero coverage). |
| **Next**       | 1. **Fairway iOS:** field walk — open audit sheet per head, enter arc/radius/nozzle/GPM, tick "Head cleared". 2. **Fairway iOS:** `fieldGPM` backward-compat decode test (see `FairwayBlobTests` pattern). 3. **Fairway iOS:** map pin fine-tuning after confirming device location. 4. **clarity-budget-web Session 3** — Claude money companion. 5. **Wellness:** split TrackerTab.jsx (WHI-63). |
| **Blockers**   | None — iOS 17.2 simruntime is mounted, device install works. |
| **Last touch** | 2026-04-25 — Fairway iOS session 3: audit sheet pre-fill + estimates + GPM field + measurement guide; Phase 3 property coords migration (40.3330→40.3004); map camera 200 m. Device install EXIT:0. |
