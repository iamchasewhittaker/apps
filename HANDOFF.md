...
| **Focus**      | **Gmail Forge × Job Search HQ alignment shipped (2026-04-28):** added subject-keyword matching for `JobSearch` (interview / availability / phone screen / time to chat / schedule a call) in [rules.gs](portfolio/gmail-forge/apps-script/rules.gs) + [auto-sort.gs](portfolio/gmail-forge/apps-script/auto-sort.gs); fixed Gemini system prompt to include `JobSearch` (was silently missing — AI fallback couldn't pick it); new [`healthCheck_jobSearch_()`](portfolio/gmail-forge/apps-script/auto-sort.gs) diagnostic; +3 XML whitelist entries (ashbyhq.com, linkedin.com, e.linkedin.com → 69→72 filters). [JSHQ InboxPanel](portfolio/job-search-hq/src/components/InboxPanel.jsx) `SetupGuide()` now leads with a "Using Gmail Forge?" callout. Build clean (+162 B). **Deploy step pending Chase:** `cd portfolio/gmail-forge/apps-script && npx clasp push --force` then re-import `gmail-filters.xml` in Gmail. — Prior: Wellness iOS Phase 2 #6 (live timer + active-session controls) shipped 2026-04-28. |
| **Next**       | 1. **Wellness iOS Phase 2 #5** — Tasks top-3 triage + one-thing modes (mirror `portfolio/wellness-tracker/src/tabs/TasksTab.jsx` triage pattern). 2. **Fairway iOS:** field walk — open audit sheet per head, enter arc/radius/nozzle/GPM, tick "Head cleared". 3. **clarity-budget-web v1 redesign — Step 4** (reconcile logic + first vitest suite). 4. **Shipyard:** Narrative Threads section. 5. **Wellness:** split HistoryTab.jsx (58 KB → analytics / export / AI summary). |
| **Blockers**   | None. |
| **Last touch** | 2026-04-28 — Gmail Forge × Job Search HQ alignment (subject matching + Gemini prompt + healthCheck_jobSearch_ + 3 XML whitelist entries + JSHQ SetupGuide callout). |
...
- **Branch consolidation (2026-04-27):** `main` is the trunk. The long-running `feat/job-search-v8.16` (formerly de-facto trunk), `feature/ai-categorize`, `chore/archive-rtt-web-rebrand-ios`, and `feat/rtt-ios-v1-simplify` were all fully contained in main and have been deleted locally. `origin/main` is now in sync. Future `checkpoint` commits land on main directly; if a short-lived feat branch is needed, fast-forward back into main when done.
...
- **Shared-login canonical host DEFERRED (2026-04-25):** `apps.chasewhittaker.com` (the canonical host in `SHARED_LOGIN_STRATEGY.md`) has **no DNS record** and was never wired up. It has been silently breaking magic-link sign-ins by sitting in Supabase's Site URL and pulling redirects to a dead domain (funded-web 2026-04-16, clarity-budget-web 2026-04-25). Until/unless that domain is actually purchased + DNS-configured, **all apps should use their own Vercel domains** (set Site URL + Redirect URLs allowlist in Supabase Dashboard accordingly). Apps that still use OTP/OAuth must verify their deploy domain is in the Redirect URLs allowlist — Supabase silently falls back to Site URL if it isn't. See `portfolio/clarity-budget-web/LEARNINGS.md` 2026-04-25 for the full postmortem.
