...
| **Focus**      | **Fairway iOS Phase 2 shipped (2026-04-28):** `GrassSubZone` model + `ZoneData.subZones` (backward-compat decoder) + Z2 seeded with Main grass (700 sq ft, 12 heads, 12 min) and Park strip (328 sq ft, 6 heads, 16 min) + sub-zone chip filter in `ZoneDetailView` + runtime card in `ScheduleView` + 2 new tests (62 total). Device install on iPhone 12 Pro Max verified. |
...
| **Blockers**   | None. |
| **Last touch** | 2026-04-28 — Fairway iOS Phase 2 (GrassSubZone) shipped; 62 tests; device install verified. |
